...
int main(void)
{
    ...
    int fd_memory = 20; // There is no descriptor, no special file, processes can only inherit this memory mapping.
    ...
    if (!pid) {
        // Child.
        char* map_test = mmap(NULL, shm_size, prot, flags, fd_memory, 0);

        sleep(nap_time_seconds);
        print_table(map_test, shm_size);
        sleep(2 * nap_time_seconds);
        print_table(map_test, shm_size);
        ASSERT_SYS_OK(munmap(map_test, shm_size)); // Not needed, will disappear with process.
        return 0;
    }
