...
void initializeStorage(struct SharedStorage* s) {
    s->put = 0;
    s->take = 0;
    ASSERT_SYS_OK(sem_init(&s->mutex, 1, 1));
    ASSERT_SYS_OK(sem_init(&s->sem_filled_slots, 1, 0));
    ASSERT_SYS_OK(sem_init(&s->sem_free_slots, 1, BUFFSIZE));
